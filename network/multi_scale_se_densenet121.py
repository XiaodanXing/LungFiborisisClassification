...
class MultiScaleSeDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        super(MultiScaleSeDenseNet, self).__init__()
        ...
        se_dict['classifier.weight'].fill_(1)
        se_dict['classifier.bias'].zero_()

        # Multi scale part
        self.load_state_dict(se_dict)
        self.se_dense_Front = nn.Sequential(*list(self.features.children())[:9])
        # self.se_transition_Front = nn.Sequential(*list(list(self.features.children())[9].children())[:2])
        # self.se_transition_Back = nn.Sequential(*list(list(self.features.children())[9].children())[2:])
        self.se_dense_Back = nn.Sequential(*list(self.features.children())[9:])
        self.conv = nn.Conv2d(1024, 512, 1, 1, groups=32, bias=False)
        self.upsample = nn.UpsamplingBilinear2d(scale_factor=2)
        self.up_conv = nn.Conv2d(1024, 512, 1, 1, groups=32, bias=False)
        # self.up_conv = nn.Conv2d(1024, 512, 1, 1, padding=1, groups=32, bias=False)

        # self.bn = nn.BatchNorm2d(1024)

        self.bn = nn.BatchNorm2d(512)
        self.bn_up = nn.BatchNorm2d(512)
        self.relu = nn.ReLU(inplace=True)
        self.relu_up = nn.ReLU(inplace=True)

        self.num_ftrs = self.classifier.in_features
        self.num_classes = num_classes
        self.linear = nn.Linear(self.num_ftrs, self.num_classes)

    def forward(self, x):
        x = self.se_dense_Front(x)
        x0 = self.conv(x)
        x0 = self.bn(x0)
        x0 = self.relu(x0)
        # x00 = self.final_SE(x0)
        # x0 = x0*x00
        # feature1 = self.relu(x0)
        # out1 = F.adaptive_avg_pool2d(x0, 1).view(x0.size(0), -1)

        x1 = self.se_dense_Back(x)
        x1 = F.relu(x1, inplace=True)
        x1 = self.upsample(x1)
        x1 = self.up_conv(x1)
        x1 = self.bn_up(x1)
        x1 = self.relu_up(x1)
        # x10 = self.final_SE(x1)
        # x1 = x1*x10
        # feature2 = self.relu_up(x1)
        # out2 = F.adaptive_avg_pool2d(x1, 1).view(x1.size(0), -1)
        # out = torch.cat([out1, out2], dim=1)
        out = torch.cat([x0, x1], dim=1)
        feature_se = self.final_SE(out)
        out = feature_se * out
